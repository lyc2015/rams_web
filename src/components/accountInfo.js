...
class BankInfo extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.employeeNo !== null && this.props.employeeNo !== undefined) {
      //社員の場合
      employeeOrCustomerNo = this.props.employeeNo;
      var employeeName =
        this.props.employeeFristName + "" + this.props.employeeLastName;
      if (
        this.props.employeeFristName === undefined &&
        this.props.employeeLastName === undefined
      ) {
        employeeName = "";
      }
      this.setState({
    ...
    if (!$.isEmptyObject(accountInfo)) {
      ...
      if (actionType !== "insert") {
        ...
        //画面データの検索
        axios
          .post(this.state.serverIP + "bankInfo/init", onloadMol)
          .then((resultMap) => {
            if (resultMap.data.accountInfoMod.length > 0) {
              this.giveValue(resultMap.data.accountInfoMod[0]);
              if (
                resultMap.data.accountInfoMod[0]["accountTypeStatus"] === "0"
              ) {
                $("#futsu").attr("checked", true);
              } else if (
                resultMap.data.accountInfoMod[0]["accountTypeStatus"] === "1"
              ) {
            ...
            if (
              actionType === "update" &&
              resultMap.data.accountInfoMod.length > 0
            ) {
  ...
  giveValue = (accountInfoMod) => {
    ...
    if (
      accountInfoMod.bankBranchName === undefined &&
      this.state.bankBranchDrop.find(
        (v) => v.code === accountInfoMod.bankBranchCode
      ) !== undefined
    ) {
      this.setState({
        bankBranchName: this.state.bankBranchDrop.find(
          (v) => v.code === accountInfoMod.bankBranchCode
        ).name,
      });
    }
  ...
  getBankBranchInfo(noORname) {
    ...
    if ($("#" + noORname + "").val() !== "") {
      axios
        .post(this.state.serverIP + "getBankBranchInfo", sendMap)
        .then((resultMap) => {
  ...
  accountTokuro() {
    ...
    if (result && bankInfoJs.torokuCheck()) {
      ...
      if (this.state.actionType === "update") {
        axios
          .post(this.state.serverIP + "bankInfo/update", accountInfo)
          .then((result) => {
      ...
      this.setState({
        errorsMessageShow: true,
        errorsMessageValue: "口座名義人をカタカナで入力してください",
      });
    } else if (!bankInfoJs.torokuCheck()) {
      this.setState({
        errorsMessageShow: true,
        errorsMessageValue: "銀行関連を入力してください",
      });
    }
  }
  /**
  ...
  deleteAccount = () => {
    ...
    if (a) {
      ...
      accountInfo["employeeOrCustomerNo"] = this.state.employeeOrCustomerNo;
      axios
        .post(this.state.serverIP + "bankInfo/delete", accountInfo)
        .then((result) => {
          if (result.data.result) {
            this.setState({
              myToastShow: true,
              ...
              <Col>
                <InputGroup size="sm" className="mb-3">
                  ...
                  <Autocomplete
                    ...
                    value={
                      this.state.bankBranchDrop.find(
                        (v) => v.name === this.state.bankBranchName
                      ) || {}
                    }
                    ...
                    renderInput={(params) => (
                      <div ref={params.InputProps.ref}>
                        <input
                          ...
                          type="text"
                          {...params.inputProps}
                          className="auto form-control Autocompletestyle-accountInfo"
                        />
                      </div>
                    )}
                  />
              ...
              <Col>
                <InputGroup size="sm" className="mb-3">
                  ...
                  <Autocomplete
                    ...
                    value={
                      this.state.bankBranchDrop.find(
                        (v) => v.code === this.state.bankBranchCode
                      ) || {}
                    }
                    options={this.state.bankBranchDrop}
            ...
            <Row>
              ...
              <Col>
                <InputGroup size="sm" className="mb-3">
                  ...
                  <OverlayTrigger
                    overlay={
                      <Tooltip id="tips">片仮名で入力してください</Tooltip>
                    }
                  >
                    <Form.Control
                      placeholder="例：カタカナ"
                      onChange={bankInfoJs.checkAccountName}
                      readOnly
                      id="accountName"
                      maxLength="20"
                      name="accountName"
            ...
            <Row>
              <Col className="text-center">
                ...
                {actionType === "update" ? (
                  <Button
                    size="sm"
                    onClick={this.accountTokuro.bind(this)}
                    variant="info"
                    id="toroku"
                    type="button"
                  >
