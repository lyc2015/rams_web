...
class CustomerInfoSearch extends Component {
  ...
  search = () => {
    ...
    customerInfoMod["capitalStockFront"] = utils.deleteComma(
      $("#capitalStockFront").val()
    );
    customerInfoMod["capitalStockBack"] = utils.deleteComma(
      $("#capitalStockBack").val()
    );
    ...
    if (this.state.contactDateFlag) {
      customerInfoMod["contactDate"] = utils.formateDate(
    ...
    customerInfoMod["basicContract"] =
      $("#basicContract").val() === "" ? null : $("#basicContract").val();
    customerInfoMod["response"] =
      $("#response").val() === "" ? null : $("#response").val();
    customerInfoMod["representative"] = $("#representative").val();
    customerInfoMod["listedCompanyFlag"] = $("#listedCompanyFlag").val();
    axios
      .post(
        this.state.serverIP + "customerInfoSearch/customerSearch",
        customerInfoMod
      )
      .then((result) => {
        if (
          ...
          this.setState(
            ...
            () => {
              if (
                ...
                this.state.customerNoForPageChange !== undefined
              ) {
  ...
  onDeleteRow = () => {
    ...
    if (a) {
      ...
      axios
        .post(
          this.state.serverIP + "customerInfoSearch/delete",
          customerInfoMod
        )
        .then((result) => {
          if (result.data === 0) {
            ...
            this.setState({
              errorsMessageShow: true,
              errorsMessageValue: "お客様が現場に使われました",
            });
          } else if (result.data === 3) {
            this.setState({
              errorsMessageShow: true,
              errorsMessageValue: "上位お客様の下位お客様が複数ある",
            });
          } else if (result.data === 4) {
            this.setState({
              errorsMessageShow: true,
              errorsMessageValue:
                "上位お客様の下位お客様が複数あるの場合でも、お客様の削除が失敗し",
            });
          }
        })
        .catch((error) => {
  ...
  shuseiTo = (actionType) => {
    ...
    sendValue["capitalStockFront"] = utils.deleteComma(
      $("#capitalStockFront").val()
    );
    sendValue["capitalStockBack"] = utils.deleteComma(
      $("#capitalStockBack").val()
    );
    sendValue["topCustomerNo"] = this.state.topCustomerCode;
    sendValue["stationCode"] = this.state.stationCode;
    sendValue["businessStartDate"] = utils.formateDate(
      this.state.businessStartDate,
      false
    );
        ...
        <Form id="conditionForm">
          ...
          <Row>
            <Col sm={3}>
              <InputGroup size="sm">
                <InputGroup.Prepend>
                  <InputGroup.Text id="fiveKanji" /*style={{ width: "8rem" }}*/>
                    ランキング
                  </InputGroup.Text>
                </InputGroup.Prepend>
            ...
            <Col sm={3}>
              <InputGroup size="sm" className="mb-3">
                ...
                <Form.Control
                  placeholder="例：100"
                  id="capitalStockFront"
                  name="capitalStockFront"
                  value={capitalStockFront}
                ...
                <Form.Control
                  placeholder="例：100"
                  id="capitalStockBack"
                  name="capitalStockBack"
                  value={capitalStockBack}
                ...
                <Autocomplete
                  ...
                  value={
                    this.state.topCustomerDrop.find(
                      (v) => v.code === this.state.topCustomerCode
                    ) || {}
                  }
                  onChange={(event, values) =>
          ...
          <Row>
            ...
            <Col sm={3}>
              <InputGroup size="sm" className="mb-3 flexWrapNoWrap">
                ...
                <Form.Control
                  ...
                  disabled={this.state.contactDateFlag}
                >
            ...
            <Col sm={3}>
              <InputGroup size="sm" className="mb-3">
                ...
                <Form.Control
                  placeholder="例：12"
                  value={traderPersonFront}
                  disabled={this.state.contactDateFlag}
                  onChange={(e) => this.vNumberChange(e, "traderPersonFront")}
                ...
                <Form.Control
                  ...
                  disabled={this.state.contactDateFlag}
                  onChange={(e) => this.vNumberChange(e, "traderPersonBack")}
                  id="traderPersonBack"
                  name="traderPersonBack"
                />
              </InputGroup>
            </Col>
          </Row>
        ...
        <Form>
          ...
          <Col sm={12}>
            ...
            >
              ...
              <TableHeaderColumn
                dataField="capitalStock"
                tdStyle={{ padding: ".45em" }}
                width="126"
                dataFormat={this.addMarkCapitalStock}
              >
                資本金(百万)
              </TableHeaderColumn>
              <TableHeaderColumn
                dataField="paymentSiteName"
                tdStyle={{ padding: ".45em" }}
                width="110"
              >
                支払サイト
