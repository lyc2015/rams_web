...
class salesPoint extends React.Component {
    ...
    switch (str) {
      case "start":
        if (
          ...
          typeof this.state.customerNo !== "undefined"
        ) {
  ...
  downloadPDF = () => {
    ...
    axios
      .post(
        this.state.serverIP + "SalesPointController/downloadPDF",
        salesPointSetModel
      )
      .then((resultMap) => {
        if (resultMap.data) {
  ...
  select = (date, str) => {
    ...
    axios
      .post(this.state.serverIP + "getPointInfoNew", salesPointSetModel)
      .then((response) => {
        if (response.data != null) {
          if (response.data.length > 0) {
            ...
            for (let i in response.data) {
              if (
                response.data[i].point !== null &&
                response.data[i].point !== ""
              )
                pointAll += Number(response.data[i].point);
              if (
                response.data[i].specialsalesPoint !== null &&
                response.data[i].specialsalesPoint !== ""
              )
              ...
              <Row>
                <Col sm={2}>
                  <InputGroup size="sm">
                    ...
                    <FormControl value={this.state.pointAll} disabled />
                  </InputGroup>
                </Col>
                <Col sm={8}></Col>
                <Col sm={2}>
                  <div style={{ float: "right" }}>
                    <Button
                      ...
                      disabled={this.state.rows > 0 ? false : true}
                    >
                      <FontAwesomeIcon icon={faDownload} /> PDF出力
                    </Button>{" "}
                  </div>
                </Col>
              </Row>
              <Row>
                <Col sm={12}>
                  ...
                  >
                    ...
                    <TableHeaderColumn
                      dataField="specialsalesPointCondition"
                      tdStyle={{ padding: ".45em" }}
                      width="170"
