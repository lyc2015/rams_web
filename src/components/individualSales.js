...
class individualSales extends React.Component {
  ...
  searchEmployee = () => {
    const empInfo = {
      ...
      startYearAndMonth: publicUtils.formateDate(
        this.state.individualSales_startYearAndMonth,
        false
      ),
      endYearAndMonth: publicUtils.formateDate(
        this.state.individualSales_endYearAndMonth,
        false
      ),
      status: "0",
    };
    ...
    axios
      .post(this.state.serverIP + "personalSales/searchEmpDetails", empInfo)
      .then((response) => {
        if (response.data.errorsMessage != null) {
          ...
          this.setState({
            workMonthCount: this.state.employeeInfoList[0].workMonthCount,
          });
          this.feeTotal();
    ...
    path = {
      ...
      state: {
        ...
        employeeClassification:
          this.state.employeeClassification === ""
            ? undefined
            : this.state.employeeClassification,
        employeeFormCodes:
          this.state.employeeFormCodes === ""
            ? undefined
            : this.state.employeeFormCodes,
        occupationCodes:
          this.state.occupationCodes === ""
            ? undefined
            : this.state.occupationCodes,
        kadou: this.state.kadou === "" ? undefined : this.state.kadou,
        utilPricefront:
          this.state.utilPricefront === ""
            ? undefined
            : this.state.utilPricefront,
        utilPriceback:
          this.state.utilPriceback === ""
            ? undefined
            : this.state.utilPriceback,
        salaryfront:
          this.state.salaryfront === "" ? undefined : this.state.salaryfront,
        salaryback:
          this.state.salaryback === "" ? undefined : this.state.salaryback,
        grossProfitFront:
          this.state.grossProfitFront === ""
            ? undefined
            : this.state.grossProfitFront,
        grossProfitBack:
          this.state.grossProfitBack === ""
            ? undefined
            : this.state.grossProfitBack,
  ...
  feeTotal = () => {
    ...
    for (var i = 0; i < this.state.employeeInfoList.length; i++) {
      paymentCal =
        parseInt(this.state.employeeInfoList[i].salary) +
        parseInt(this.state.employeeInfoList[i].transportationExpenses) +
        parseInt(this.state.employeeInfoList[i].insuranceFeeAmount) +
        parseInt(this.state.employeeInfoList[i].bonusFee) +
        parseInt(this.state.employeeInfoList[i].deductionsAndOvertimePay) +
        parseInt(this.state.employeeInfoList[i].leaderAllowanceAmount) +
        parseInt(this.state.employeeInfoList[i].otherAllowanceAmount) +
        parseInt(this.state.employeeInfoList[i].introductionAllowance);
    ...
    if (this.props.location.state !== undefined) {
      if (this.props.location.state.sendValue !== undefined) {
        ...
        this.setState(
          {
            ...
            individualSales_startYearAndMonth:
              sendValue.individualSales_startYearAndMonth,
            individualSales_endYearAndMonth:
              sendValue.individualSales_endYearAndMonth,
        ...
        if (location.state) {
          ...
          monthlySales_startYearAndMonth =
            location.state.monthlySales_startYearAndMonth;
          monthlySales_endYearAndMonth =
            location.state.monthlySales_endYearAndMonth;
          ...
          this.setState(
            {
              ...
              employeeName:
                rowSelectemployeeName + "(" + rowSelectemployeeNo + ")",
  ...
  deductionsAndOvertimePayOfUnitPriceAddComma(cell, row) {
    if (
      ...
      let formatDeductionsAndOvertimePayOfUnitPrice = publicUtils.addComma(
      ...
      if (row.deductionsAndOvertimePayOfUnitPrice < 0) {
        return (
          <div style={{ color: "red" }}>
            {formatDeductionsAndOvertimePayOfUnitPrice}
          </div>
        );
      }
  ...
  insuranceFeeAmountAddComma(cell, row) {
    if (row.insuranceFeeAmount === null || row.insuranceFeeAmount === "0") {
      ...
      let formatInsuranceFeeAmount = publicUtils.addComma(
        row.insuranceFeeAmount,
        false
      );
  ...
  scheduleOfBonusAmountAddComma(cell, row) {
    if (row.bonusFee === null || row.bonusFee === "0") {
      ...
      let formatScheduleOfBonusAmount = publicUtils.addComma(
        row.bonusFee,
        false
      );
  ...
  deductionsAndOvertimePayAddComma = (cell, row) => {
    if (
      row.deductionsAndOvertimePay === null ||
      row.deductionsAndOvertimePay === "0"
    ) {
      return;
    } else {
      let formatDeductionsAndOvertimePay = publicUtils.addComma(
        row.deductionsAndOvertimePay,
        false
      );
      if (row.deductionsAndOvertimePay < 0) {
        return (
          <div style={{ color: "red" }}>{formatDeductionsAndOvertimePay}</div>
        );
      } else {
  ...
  AllowanceAmountAddComma(cell, row) {
    if (row.allowanceAmount === null || row.allowanceAmount === "0") {
      ...
      let formatAllowanceAmount = publicUtils.addComma(
        row.allowanceAmount,
        false
      );
    ...
    if (row.grosProfits === null || row.grosProfits === "0") {
      ...
      if (row.dailyCalculationStatus == "0") {
        if (row.admissionStartDate.substring(0, 6) == row.onlyYandM) {
          ...
          var monthDayCount = new Date(
            row.admissionStartDate.substring(0, 4),
            row.admissionStartDate.substring(4, 6),
            0
          ).getDate();
          var a = row.admissionStartDate.substring(0, 4);
          var b = row.admissionStartDate.substring(4, 6);
          for (
            var i = row.admissionStartDate.substring(6, 8);
            i <= monthDayCount;
            i++
          ) {
            if (i < 10) {
          ...
          var dailySalary = parseInt(
            (workdayCount / totalworkdayCount) * row.unitPrice
          );

          var dailySalayCal = dailySalary;

          var calgrosProfits =
            parseInt(dailySalayCal) +
            parseInt(row.deductionsAndOvertimePayOfUnitPrice) -
            (parseInt(row.salary) +
              parseInt(row.insuranceFeeAmount) +
              parseInt(row.bonusFee) +
              parseInt(row.allowanceAmount) +
              parseInt(row.deductionsAndOvertimePay));
          let returnItem = cell;
        ...
        if (
          ...
          row.admissionEndDate.substring(0, 6) === row.onlyYandM
        ) {
          var monthDayCount = new Date(
            row.admissionEndDate.substring(0, 4),
            row.admissionEndDate.substring(4, 6),
            0
          ).getDate();
  ...
  employeeStatusChange = (event) => {
    ...
    if (value === "0") {
      ...
      for (let i in tempEmployeeList) {
        if (
          tempEmployeeList[i].code.substring(0, 2) !== "BP" &&
          tempEmployeeList[i].code.substring(0, 2) !== "SP" &&
          tempEmployeeList[i].code.substring(0, 2) !== "SC"
        ) {
          newEmpInfoList.push(tempEmployeeList[i]);
        }
      }
  ...
  workDaysCal = (cell, row) => {
    ...
    if (row.dailyCalculationStatus == "0") {
      ...
      var yearAndMonth =
        ...
        String(month.toString()[1] ? month : "0" + month);
      if (row.onlyYandM > yearAndMonth && dailySalay != 0) dailySalay += "(予)";
      returnItem = dailySalay;
      if (returnItem == 0) {
        returnItem = "";
      }
  ...
  unitPriceTotalCal = (empList) => {
    ...
    for (var m = 0; m < empList.length; m++) {
      ...
      if (empList[m].dailyCalculationStatus == "0") {
        if (startDate.substring(0, 6) == empList[m].onlyYandM) {
          if (startDate.substring(6, 8) == "01") {
            if (deductionsAndOver != null) {
              unitPriceTotal =
                parseInt(unitPriceTotal) +
                parseInt(unPrice) +
                parseInt(deductionsAndOver);
            } else {
            ...
            dailySalary = parseInt(
              (workdayCount / totalworkdayCount) * unPrice
            );
            if (deductionsAndOver != null) {
              var dailySalayCal =
                parseInt(dailySalary) + parseInt(deductionsAndOver);
              unitPriceTotal =
                parseInt(unitPriceTotal) + parseInt(dailySalayCal);
            } else {
        ...
        if (
          endDate !== null &&
          startDate.substring(0, 6) !== endDate.substring(0, 6) &&
          endDate.substring(0, 6) == empList[m].onlyYandM
        ) {
          var monthDayCount = new Date(
          ...
          if (endDate.substring(6, 8) == monthDayCount) {
            ...
            if (deductionsAndOver != null) {
              dailySalarys =
                parseInt(dailySalary) + parseInt(deductionsAndOver);
              unitPriceTotal =
                parseInt(unitPriceTotal) + parseInt(dailySalarys);
            } else {
        ...
        if (
          startDate.substring(0, 6) != empList[m].onlyYandM &&
          endDate.substring(0, 6) != empList[m].onlyYandM
        ) {
          if (deductionsAndOver != null) {
    ...
    this.setState({
      unitPriceTotal: publicUtils.addComma(unitPriceTotal, false),
    });
                ...
                <strong>
                  ...
                  >
                    ...
                    <tr>
                      <td>交通費</td>
                      <td>
                        {publicUtils.addComma(
                          row.transportationExpenses,
                          false
                        )}
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td>
                        {publicUtils.addComma(row.otherAllowanceAmount, false)}
                      </td>
                    </tr>
                    ...
                    <tr>
                      <td>住宅</td>
                      <td>
                        {publicUtils.addComma(row.introductionAllowance, false)}
                      </td>
                    </tr>
                    <tr>
                      <td>リーダー</td>
                      <td>
                        {publicUtils.addComma(row.leaderAllowanceAmount, false)}
                      </td>
                    </tr>
                    <tr>
                      <td>関連社員</td>
      ...
      <div>
          ...
          <Col>
            <InputGroup size="sm" className="mb-3">
              ...
              <FormControl
                ...
                onChange={this.yearAndMonthChange.bind(this)}
              />
            </InputGroup>
          </Col>
          <Col></Col>
          ...
          <Col>
            <InputGroup size="sm" className="mb-3">
              ...
              <Form.Control
                as="select"
                size="sm"
                onChange={this.kadouChange.bind(this)}
                name="kadou"
                value={this.state.kadou}
                autoComplete="off"
              >
                <option value=""></option>
        ...
        <Row>
          ...
          <Col>
            <InputGroup size="sm">
              ...
              <FormControl value={this.state.workMonthCount} disabled />
            </InputGroup>
          </Col>
          ...
          <Col>
            <InputGroup size="sm">
              ...
              <FormControl value={this.state.unitPriceTotal} disabled />
            </InputGroup>
          </Col>
          <Col>
            <InputGroup size="sm">
              ...
              <FormControl value={this.state.paymentTotal} disabled />
            </InputGroup>
          </Col>
          <Col>
            <InputGroup size="sm">
              ...
              <FormControl
                value={
                  this.state.unitPriceTotalnoComma -
                  this.state.paymentTotalnoComma
                    ? publicUtils.addComma(
                        this.state.unitPriceTotalnoComma -
                          this.state.paymentTotalnoComma,
                        false
        ...
        <Col>
          <div>
            <BootstrapTable
              selectRow={selectRow}
              data={this.state.employeeInfoList}
              pagination={true}
            ...
            >
              ...
              <TableHeaderColumn
                ...
                width="110"
                dataFormat={
                  this.unitPriceAddComma
                } /*dataFormat={this.workDaysCal.bind(this)}*/
              >
                単価
              </TableHeaderColumn>
              <TableHeaderColumn
                ...
                width="108"
                dataFormat={this.deductionsAndOvertimePayOfUnitPriceAddComma}
              >
                控/残(単価)
              </TableHeaderColumn>
              <TableHeaderColumn
                ...
                dataField="salary"
                width="110"
                dataFormat={this.salaryAddComma}
              >
                基本支給
              </TableHeaderColumn>
              <TableHeaderColumn
                ...
                dataFormat={this.deductionsAndOvertimePayAddComma.bind(this)}
              >
                控/残
              </TableHeaderColumn>
              <TableHeaderColumn
                tdStyle={{ padding: ".45em" }}
                hidden={this.state.bpFlag}
                dataField="insuranceFeeAmount"
                width="100"
                dataFormat={this.insuranceFeeAmountAddComma}
              >
                社会保険
              </TableHeaderColumn>
              <TableHeaderColumn
                ...
                dataField="bounsFee"
                width="100"
                dataFormat={this.scheduleOfBonusAmountAddComma}
              >
                ボーナス
              </TableHeaderColumn>
              <TableHeaderColumn
                ...
                width="90"
                dataFormat={this.allowanceDetail.bind(this)}
              >
                諸費用
              </TableHeaderColumn>
              <TableHeaderColumn
                tdStyle={{ padding: ".45em" }}
                hidden={!this.state.bpFlag}
                dataField="bpBelongCustomer"
                width="290"
                dataFormat={this.bpBelongCustomerDetail.bind(this)}
              >
                所属会社
              </TableHeaderColumn>
              <TableHeaderColumn
                tdStyle={{ padding: ".45em" }}
                dataField="grosProfits"
                width="90"
                dataFormat={this.grosProfitsAddComma}
              >
                粗利
              </TableHeaderColumn>
            </BootstrapTable>
