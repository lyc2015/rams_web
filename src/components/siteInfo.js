...
class siteInfo extends Component {
  ...
  onchangeworkState = (event) => {
    if (event.target.value === "0") {
      ...
      this.setState({
        ...
        workStateFlag: true,
        systemName: "休暇",
        customerNo: this.state.customerMaster.find((v) => v.name === "LYC")
          .code,
        remark: "",
        scheduledEndDate: this.state.scheduledEndDateForSave,
    ...
    if (date !== null) {
      if (date.getDate() > 2) {
        ...
        if (
          ...
          if (
            new Date(
              this.state.admissionEndDate.getFullYear(),
    ...
    if (date !== null) {
      if (
        ...
        if (
          ...
          this.state.admissionStartDate !== undefined
        ) {
  ...
  componentDidMount() {
    ...
    if (
      ...
      if (
        ...
        this.props.location.state.currPage !== undefined &&
      ...
      axios
        .post(this.state.serverIP + "getSiteInfo", { employeeName: employeeNo })
        .then((response) => {
          if (
    ...
    this.setState(
      ...
      () => {
        ...
        if (typteOfContract === "4") {
          let customerNo =
            this.state.customerMaster.find((v) => v.name === "LYC").code || {};
          this.setState({
  ...
  getRowData = (row) => {
    this.setState({
      ...
      admissionEndDate:
        row.admissionEndDate === null
          ? ""
          : new Date(publicUtils.converToLocalTime(row.admissionEndDate, true)),
      ...
      developLanguageCode:
        row.developLanguageCode === null ? "" : row.developLanguageCode,
      developLanguageCode2:
        row.developLanguageCode2 === null ? "" : row.developLanguageCode2,
      ...
      typeOfIndustryCode:
        row.typeOfIndustryCode === null ? "" : row.typeOfIndustryCode,
      typteOfContract:
        row.typteOfContractCode === null ? "" : row.typteOfContractCode,
      remark: row.remark === null ? "" : row.remark,
      scheduledEndDate:
        row.scheduledEndDate === null
          ? ""
          : publicUtils.converToLocalTime(row.scheduledEndDate, false),
      scheduledEndDateForSave:
        row.scheduledEndDate === null
          ? ""
          : publicUtils.converToLocalTime(row.scheduledEndDate, false),
    ...
    if (
      publicUtils.converToLocalTime(row.admissionStartDate, true).getDate() > 2
    ) {
      this.setState({
        dailyCalculationStatusFlag: false,
        deleteFlag: false,
      ...
      row.admissionEndDate !== null &&
      row.admissionEndDate !== undefined
    ) {
      if (
        new Date(
          publicUtils
    ...
    if (
      row.workDate ===
      this.state.siteData[this.state.siteData.length - 1].workDate
    ) {
      this.setState({
        pageDisabledFlag: false,
        disabledFlag: false,
  ...
  handleRowSelect = (row, isSelected) => {
    if (isSelected) {
      ...
      if (
        publicUtils.converToLocalTime(row.admissionStartDate, true).getDate() >
        2
      ) {
        this.setState({
          dailyCalculationStatusFlag: false,
  ...
  tokuro = () => {
    ...
    siteModel["related1Employees"] = publicUtils.valueGetText(
      this.state.related1Employees,
      this.state.employeeInfoAll
    );
    siteModel["related2Employees"] = publicUtils.valueGetText(
      this.state.related2Employees,
      this.state.employeeInfoAll
    );
    siteModel["related3Employees"] = publicUtils.valueGetText(
      this.state.related3Employees,
      this.state.employeeInfoAll
    );
    siteModel["related4Employees"] = publicUtils.valueGetText(
      this.state.related4Employees,
      this.state.employeeInfoAll
    );
    ...
    siteModel["scheduledEndDate"] = publicUtils.formateDate(
    ...
    if (this.state.siteData.length > 0) {
      siteModel["checkDate"] =
        this.state.siteData[this.state.siteData.length - 1].admissionEndDate;
    } else {
  ...
  addCommaUnitPrice = (cell, row) => {
    let unitPrice =
      row.unitPrice === ""
        ? ""
        : (row.unitPrice / 10000).toFixed(1).replace(".0", "");
    return unitPrice;
  };
            ...
            <Form.Group>
              <Row>
                ...
                <Col sm={2} lg={3}>
                  <InputGroup size="sm" className="mb-3">
                    ...
                    <Form.Control
                      className="auto form-control siteInfo-workState"
                      as="select"
                      id="workState"
                      name="workState"
                      value={workState}
                      onChange={this.onchangeworkState}
                ...
                <Col sm={3}>
                  <InputGroup size="sm" className="mb-3">
                    ...
                    <FormControl
                      ...
                      disabled={
                        this.state.workState === "3" || pageDisabledFlag
                          ? true
                          : false
                      }
                    />
              ...
              <Row>
                <Col sm={3}>
                  ...
                  <InputGroup size="sm" className="mb-3">
                    ...
                    <Autocomplete
                      ...
                      value={
                        this.state.developLanguageMaster.find(
                          (v) => v.code === this.state.developLanguageCode
                        ) || {}
                      }
                      onChange={(event, values) =>
                    ...
                    <Autocomplete
                      ...
                      value={
                        this.state.developLanguageMaster.find(
                          (v) => v.code === this.state.developLanguageCode2
                        ) || {}
                      }
                      onChange={(event, values) =>
                ...
                <Col sm={3}>
                  <InputGroup size="sm" className="mb-3">
                    ...
                    <Form.Control
                      ...
                      id="payOffRange2"
                      name="payOffRange2"
                      value={payOffRange1 === "0" ? "0" : payOffRange2}
                      autoComplete="off"
                      disabled={
                        payOffRange1 === "0"
                          ? true
                ...
                <Col sm={3}>
                  <InputGroup size="sm" className="mb-3">
                    ...
                    <Form.Control
                      className="auto form-control siteInfo-siteRoleCode"
                      as="select"
                      id="siteRoleCode"
                      name="siteRoleCode"
                      onChange={this.onchangeSiteRoleCode}
                      value={siteRoleCode}
                      autoComplete="off"
                      disabled={pageDisabledFlag}
                    >
                    ...
                    <Form.Control
                      ...
                      autoComplete="off"
                      disabled={
                        pageDisabledFlag ? true : workStateFlag ? true : false
                      }
                    >
                ...
                <Col sm={6}>
                  <InputGroup size="sm" className="mb-3">
                    ...
                    <Autocomplete
                      ...
                      name="employeeName"
                      value={
                        this.state.employeeInfoAll.find(
                          (v) => v.code === this.state.related1Employees
                        ) || ""
                      }
                      ...
                      disabled={
                        this.state.relatedEmployeesFlag
                          ? pageDisabledFlag
                            ? true
                            : false
                          : true
                      ...
                      renderInput={(params) => (
                        <div ref={params.InputProps.ref}>
                          <input
                            type="text"
                            {...params.inputProps}
                            className="auto form-control Autocompletestyle-siteInfo-relatedEmployees"
                          />
                        </div>
                      )}
                    />
                    <Autocomplete
                      ...
                      value={
                        this.state.employeeInfoAll.find(
                          (v) => v.code === this.state.related2Employees
                        ) || ""
                      }
                      options={this.state.employeeInfoAll}
                      disabled={
                    ...
                    <Autocomplete
                      ...
                      value={
                        this.state.employeeInfoAll.find(
                          (v) => v.code === this.state.related3Employees
                        ) || ""
                      }
                      options={this.state.employeeInfoAll}
                      disabled={
                    ...
                    <Autocomplete
                      ...
                      value={
                        this.state.employeeInfoAll.find(
                          (v) => v.code === this.state.related4Employees
                        ) || ""
                      }
                      options={this.state.employeeInfoAll}
                      disabled={
              ...
              <div style={{ textAlign: "center" }}>
                <Button
                  ...
                  size="sm"
                  onClick={
                    this.state.updateFlag === true ? this.tokuro : this.update
                  }
                  variant="info"
                  id="toroku"
                  type="button"
                ...
                >
                  ...
                  {this.state.updateFlag === true ? "登録" : "修正"}
                </Button>{" "}
                <Button
                  size="sm"
                  type="reset"
          ...
          <Row>
            <Col sm={12}>
              ...
              >
                ...
                ></TableHeaderColumn>
                <TableHeaderColumn
                  dataField="admissionEndDate"
                  hidden={true}
                ></TableHeaderColumn>
                <TableHeaderColumn
                  dataField="payOffRange1"
                  hidden={true}
                ></TableHeaderColumn>
                <TableHeaderColumn
                  dataField="payOffRange2"
                  hidden={true}
                ></TableHeaderColumn>
                <TableHeaderColumn
                  dataField="related1Employees"
                  hidden={true}
                ></TableHeaderColumn>
                <TableHeaderColumn
                  dataField="related2Employees"
                  hidden={true}
                ></TableHeaderColumn>
                <TableHeaderColumn
                  dataField="related3Employees"
                  hidden={true}
                ></TableHeaderColumn>
                <TableHeaderColumn
                  dataField="related4Employees"
                  hidden={true}
                ></TableHeaderColumn>
                <TableHeaderColumn
                  dataField="typeOfIndustryName"
                  hidden={true}
                ></TableHeaderColumn>
                <TableHeaderColumn
                  dataField="topCustomerName"
                  hidden={true}
                ></TableHeaderColumn>
                <TableHeaderColumn
                  dataField="workState"
                ...
                ></TableHeaderColumn>
                <TableHeaderColumn
                  dataField="dailyCalculationStatus"
                  hidden={true}
                ></TableHeaderColumn>
                <TableHeaderColumn
                  dataField="remark"
