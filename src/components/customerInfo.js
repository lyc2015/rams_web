...
class CustomerInfo extends Component {
  ...
  async componentDidMount() {
    ...
    await axios
      .post(this.state.serverIP + "customerInfo/onloadPage", customerInfoMod)
      .then((resultMap) => {
        var customerInfoMod;
        ...
        if (actionType === "insert") {
          ...
          $("#purchasingManagersMail").val(
            customerInfoMod.purchasingManagersMail
          );
          ...
          $("#proposeClassificationCode").val(
            customerInfoMod.proposeClassificationCode
          );

          this.setState({
            businessStartDate: utils.converToLocalTime(
              customerInfoMod.businessStartDate,
              false
            ),
            establishmentDate: utils.converToLocalTime(
              customerInfoMod.establishmentDate,
              false
            ),
            customerDepartmentList: resultMap.data.customerDepartmentInfoList,
            customerDepartmentCode2: customerInfoMod.customerDepartmentCode,
            positionCode2: customerInfoMod.positionCode,
            contactDate: publicUtils.converToLocalTime(
              customerInfoMod.contactDate,
              true
            ),
            salesStaff: customerInfoMod.salesStaff,
            responseFlag:
              actionType === "detail" || customerInfoMod.basicContract !== "1"
                ? true
                : false,
            contactDateFlag:
              actionType === "detail" || customerInfoMod.response !== "1"
                ? true
                : false,
  ...
  toroku = () => {
    ...
    customerInfoMod["capitalStock"] = utils.deleteComma(
      $("#capitalStock").val()
    );
    ...
    customerInfoMod["establishmentDate"] = utils.formateDate(
      this.state.establishmentDate,
      false
    );
    customerInfoMod["businessStartDate"] = utils.formateDate(
    ...
    customerInfoMod["customerDepartmentCode"] =
      this.state.customerDepartmentCode2;
    ...
    customerInfoMod["contactDate"] = publicUtils.formateDate(
    ...
    axios
      .post(this.state.serverIP + "customerInfo/toroku", customerInfoMod)
      .then((result) => {
        if (
          ...
          store.dispatch({
            type: "UPDATE_STATE",
            dropName: "getCustomerAbbreviation",
          });
          store.dispatch({
            type: "UPDATE_STATE",
            dropName: "getCustomerNameWithMail",
          });
          store.dispatch({
            type: "UPDATE_STATE",
            dropName: "getPurchasingManagersWithMail",
          });
  ...
  onDeleteRow = (rows) => {
    ...
    if (a) {
      ...
      customerDepartmentInfoModel["customerDepartmentCode"] =
        this.state.customerDepartmentName;
      customerDepartmentInfoModel["positionCode"] = this.state.positionCode;
      customerDepartmentInfoModel["responsiblePerson"] =
        this.state.responsiblePerson;
      if (this.state.actionType === "update") {
        axios
          .post(
            this.state.serverIP + "customerInfo/customerDepartmentdelete",
            customerDepartmentInfoModel
          )
          .then((result) => {
            if (result.data) {
              this.setState({
  ...
  getCustomerDepartment = (no) => {
    this.state.customerDepartmentList[
      this.state.rowNo - 1
    ].customerDepartmentCode = no;
  ...
  getIndustry = (no) => {
    this.state.customerDepartmentList[this.state.rowNo - 1].typeOfIndustryCode =
      no;
  ...
  getTopCustomer1 = (no) => {
    this.state.customerDepartmentList[this.state.rowNo - 1].topCustomerCode =
      no;
  ...
  getLanguage1 = (no) => {
    this.state.customerDepartmentList[
      this.state.rowNo - 1
    ].developLanguageCode1 = no;
    this.formatLanguage(no);
  };
  getLanguage2 = (no) => {
    this.state.customerDepartmentList[
      this.state.rowNo - 1
    ].developLanguageCode2 = no;
    this.formatLanguage(no);
  };
  /**
   * 行追加
  ...
  meisaiToroku = () => {
    ...
    customerInfoMod["customerNo"] = $("#customerNo").val();
    customerInfoMod["actionType"] = this.state.actionType;
    customerInfoMod["customerDepartmentList"] =
      this.state.customerDepartmentList;
    axios
      .post(this.state.serverIP + "customerInfo/toroku", customerInfoMod)
      .then((result) => {
        if (
          ...
          this.setState({
            ...
            myDeleteShow: false,
            message: "明細登録成功",
            customerDepartmentList: result.data.customerDepartmentList,
          });
  ...
  render() {
    ...
    const tableSelect1 = (onUpdate, props) => (
      <TableSelect dropdowns={this} flag={5} onUpdate={onUpdate} {...props} />
    );
    const tableSelect2 = (onUpdate, props) => (
      <TableSelect dropdowns={this} flag={6} onUpdate={onUpdate} {...props} />
    );
    const tableSelect3 = (onUpdate, props) => (
      <TableSelect dropdowns={this} flag={7} onUpdate={onUpdate} {...props} />
    );
    const tableSelect4 = (onUpdate, props) => (
      <TableSelect dropdowns={this} flag={8} onUpdate={onUpdate} {...props} />
    );
    const tableSelect5 = (onUpdate, props) => (
      <TableSelect dropdowns={this} flag={9} onUpdate={onUpdate} {...props} />
    );
    const tableSelect6 = (onUpdate, props) => (
      <TableSelect dropdowns={this} flag={10} onUpdate={onUpdate} {...props} />
    );
    const tableSelect11 = (onUpdate, props) => (
      <TableSelect dropdowns={this} flag={11} onUpdate={onUpdate} {...props} />
    );
    return (
      <div>
        ...
        <div style={{ display: this.state.myUpdateShow ? "block" : "none" }}>
          <MyToast
            myToastShow={this.state.myUpdateShow}
            message={message}
            type={type}
          />
        </div>
        <div style={{ display: this.state.myDeleteShow ? "block" : "none" }}>
          <MyToast
            myToastShow={this.state.myDeleteShow}
            message={message}
            type={type}
          />
        </div>
        ...
        <div>
          <Modal
            ...
            centered
            backdrop="static"
            dialogClassName="modal-accountInfo"
            onHide={this.handleHideModal.bind(this, "bankInfo")}
            show={this.state.showBankInfoModal}
          >
              ...
              <Col sm={3}>
                <InputGroup size="sm" className="mb-3">
                  ...
                  <Autocomplete
                    ...
                    name="stationCode"
                    value={
                      this.state.stationCodeDrop.find(
              ...
              <Col sm={3}>
                <InputGroup size="sm" className="mb-3">
                  ...
                  <Autocomplete
                    ...
                    name="topCustomer"
                    value={
                      this.state.topCustomerDrop.find(
                        (v) => v.code === this.state.topCustomer
                      ) || {}
                    }
                    onChange={(event, values) =>
            ...
            <Row>
              ...
              <Col sm={3}>
                <InputGroup size="sm" className="mb-3">
                  ...
                  <Form.Control
                    maxLength="50"
                    placeholder="例：xxxxxxxxx@xxx.xxx.com"
                    id="commonMail"
                    name="commonMail"
                  />
                </InputGroup>
              ...
              <Col sm={3}>
                <InputGroup size="sm" className="mb-3">
                  ...
                  <Autocomplete
                    ...
                    name="positionCode"
                    value={
                      this.state.positionDrop.find(
                        (v) => v.code === this.state.positionCode2
                      ) || {}
                    }
                    onChange={(event, values) =>
              ...
              <Col sm={3}>
                <InputGroup size="sm" className="mb-3">
                  ...
                  <Form.Control
                    maxLength="50"
                    placeholder="例：xxxxxxxxx@xxx.xxx.com"
                    id="purchasingManagersMail"
                    name="purchasingManagersMail"
                  />
                </InputGroup>
              </Col>
              <Col sm={3}>
              ...
              <Col sm={3}>
                <InputGroup size="sm" className="mb-3">
                  ...
                  <Form.Control
                    ...
                    id="basicContract"
                    name="basicContract"
                    onChange={this.basicContractChange.bind(this)}
                  >
                  ...
                  <Form.Control
                    ...
                    id="response"
                    name="response"
                    onChange={this.responseChange.bind(this)}
                    disabled={this.state.responseFlag}
                  >
            ...
            <Col sm={4}>
              <div style={{ float: "right" }}>
                <Button
                  ...
                  id="insertRow"
                  type="button"
                  disabled={this.state.actionType === "update" ? false : true}
                >
                  <FontAwesomeIcon icon={faSave} />
                  追加
                </Button>{" "}
          ...
          <div>
            <Col sm={12}>
              <BootstrapTable
                selectRow={
                  actionType !== "detail" ? selectRow : selectRowDetail
                }
                pagination={true}
                options={options}
                deleteRow
                data={customerDepartmentList}
                insertRow
                ref="table"
              ...
              >
                ...
                <TableHeaderColumn
                  ...
                  customEditor={{ getElement: tableSelect11 }}
                >
                  Top客様
                </TableHeaderColumn>
                <TableHeaderColumn
