...
class monthlySalesSearch extends Component {
      ...
      this.setState(
        {
          ...
          utilPricefront:
            this.state.utilPricefront === "" ? "" : data.utilPricefront,
          utilPriceback:
            this.state.utilPriceback === "" ? "" : data.utilPriceback,
          salaryfront: this.state.salaryfront === "" ? "" : data.salaryfront,
          salaryback: this.state.salaryback === "" ? "" : data.salaryback,
          grossProfitFront:
            this.state.grossProfitFront === "" ? "" : data.grossProfitFront,
          grossProfitBack:
            this.state.grossProfitBack === "" ? "" : data.grossProfitBack,
  ...
  searchMonthlySales = () => {
    ...
    const monthlyInfo = {
      ...
      startYandM: publicUtils.formateDate(
        this.state.monthlySales_startYearAndMonth,
        false
      ),
      endYandM: publicUtils.formateDate(
        this.state.monthlySales_endYearAndMonth,
        false
      ),
  ...
  totalfee = () => {
    ...
    for (let i = 0; i < this.state.monthlySalesInfoList.length; i++) {
      workcount++;
      if (
        this.state.monthlySalesInfoList[i].unitPrice == null ||
        this.state.monthlySalesInfoList[i].unitPrice == ""
      ) {
        unitPirceTotal = parseInt(unitPirceTotal) + 0;
      } else {
        if (
          this.state.monthlySalesInfoList[i]
            .deductionsAndOvertimePayOfUnitPrice === null ||
          this.state.monthlySalesInfoList[i]
            .deductionsAndOvertimePayOfUnitPrice === ""
        ) {
          unitPirceTotal =
            parseInt(unitPirceTotal) +
            parseInt(this.state.monthlySalesInfoList[i].unitPrice);
        } else {
          unitPirceTotal =
            parseInt(unitPirceTotal) +
            parseInt(this.state.monthlySalesInfoList[i].unitPrice) +
      ...
      if (
        this.state.monthlySalesInfoList[i].salary == null ||
        this.state.monthlySalesInfoList[i].salary == ""
      ) {
        ...
        if (
          ...
          salaryTotal =
            salaryTotal + parseInt(this.state.monthlySalesInfoList[i].salary);
        } else {
          salaryTotal =
            salaryTotal +
            parseInt(this.state.monthlySalesInfoList[i].salary) +
            parseInt(
              this.state.monthlySalesInfoList[i].deductionsAndOvertimePay
            );
        }
      ...
      if (
        this.state.monthlySalesInfoList[i].waitingCost == null ||
        this.state.monthlySalesInfoList[i].waitingCost == ""
      ) {
        TotalNonOperation = parseInt(TotalNonOperation) + 0;
      } else {
        TotalNonOperation =
          parseInt(TotalNonOperation) +
          parseInt(this.state.monthlySalesInfoList[i].waitingCost);
      }

      if (
        this.state.monthlySalesInfoList[i].monthlyGrosProfits == null ||
        this.state.monthlySalesInfoList[i].monthlyGrosProfits == ""
      ) {
        grossProfitTotal = parseInt(grossProfitTotal) + 0;
      } else {
        grossProfitTotal =
          parseInt(grossProfitTotal) +
          parseInt(this.state.monthlySalesInfoList[i].monthlyGrosProfits);
      }
    }
    this.setState({
      unitPirceTotal: publicUtils.addComma(unitPirceTotal.toString(), false),
    });
    this.setState({
      salaryTotal: publicUtils.addComma(salaryTotal.toString(), false),
    });
    this.setState({
      TotalNonOperation: publicUtils.addComma(
        TotalNonOperation.toString(),
        false
      ),
    });
    this.setState({
      grossProfitTotal: publicUtils.addComma(
        grossProfitTotal.toString(),
        false
      ),
    });
          ...
          return (
            <div>
              <div style={{ float: "left" }}>
                {publicUtils.addComma(row.unitPrice, false)}
              </div>
      ...
      if (row.deductionsAndOvertimePay != null) {
        if (row.deductionsAndOvertimePay < 0) {
          return (
            <div>
              <div style={{ float: "left" }}>
                {publicUtils.addComma(row.salary, false)}
              </div>
              <div style={{ color: "red", float: "left" }}>
                ({publicUtils.addComma(row.deductionsAndOvertimePay, false)})
              </div>
            </div>
          );
        }
      ...
      <div>
            ...
            <Col hidden>
              <InputGroup size="sm" className="mb-3">
                ...
                <Form.Control
                  ...
                  name="employeeForms"
                  id="employeeForms"
                  value={employeeClassification === "1" ? "" : employeeForms}
                  autoComplete="off"
                  disabled={employeeClassification === "1" ? true : false}
                >
            ...
            <Col sm={3}>
              <InputGroup size="sm" className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={this.valueChange}
                  disabled={
                    this.state.employeeClassification === "1" ? true : false
                  }
                  name="employeeOccupation"
                  id="employeeOccupation"
                  value={
                    this.state.employeeClassification === "1"
                      ? ""
                      : employeeOccupation
                  }
                  autoComplete="off"
          ...
          <Row>
            <Col sm={3}>
              <InputGroup size="sm" className="mb-3">
                ...
                <FormControl
                  name="utilPricefront"
                  id="utilPricefront"
                  value={this.state.utilPricefront}
                  onChange={(e) => this.vNumberChange(e, "utilPricefront")}
                  aria-label="Small"
                  aria-describedby="inputGroup-sizing-sm"
                  placeholder=" 万円"
                />
                ...
                <FormControl
                  name="utilPriceback"
                  id="utilPriceback"
                  value={this.state.utilPriceback}
                  onChange={(e) => this.vNumberChange(e, "utilPriceback")}
                  aria-label="Small"
                  aria-describedby="inputGroup-sizing-sm"
                  placeholder=" 万円"
                />
            ...
            <Col sm={3}>
              <InputGroup size="sm" className="mb-3">
                ...
                <FormControl
                  name="salaryfront"
                  id="salaryfront"
                  value={this.state.salaryfront}
                  onChange={(e) => this.vNumberChange(e, "salaryfront")}
                  aria-label="Small"
                  aria-describedby="inputGroup-sizing-sm"
                  placeholder=" 万円"
                />
                ...
                <FormControl
                  name="salaryback"
                  id="salaryback"
                  value={this.state.salaryback}
                  onChange={(e) => this.vNumberChange(e, "salaryback")}
                  aria-label="Small"
                  aria-describedby="inputGroup-sizing-sm"
                  placeholder=" 万円"
                />
            ...
            <Col sm={3}>
              <InputGroup size="sm" className="mb-3">
                ...
                <FormControl
                  name="grossProfitFront"
                  id="grossProfitFront"
                  value={this.state.grossProfitFront}
                  onChange={(e) => this.vNumberChange(e, "grossProfitFront")}
                  aria-label="Small"
                  aria-describedby="inputGroup-sizing-sm"
                  placeholder=" 万円"
                />
                ...
                <FormControl
                  ...
                  id="grossProfitBack"
                  value={this.state.grossProfitBack}
                  onChange={(e) => this.vNumberChange(e, "grossProfitBack")}
                  aria-label="Small"
                  aria-describedby="inputGroup-sizing-sm"
                  placeholder=" 万円"
                />
          ...
          <Row className="mb-3">
            <Col className="text-center">
              <Button
                ...
                size="sm"
                id="shusei"
                onClick={this.searchMonthlySales}
              >
                <FontAwesomeIcon icon={faSearch} />
                検索
              </Button>{" "}
              <Button size="sm" variant="info" onClick={this.resetForm}>
        ...
        <Row>
          <div style={{ textAlign: "center" }}>
            <Link
              to={{
                pathname: "/subMenuManager/individualSales",
                state: {
                  actionType: "monthly",
                  backPage: "monthlySalesSearch",
                  monthlySales_startYearAndMonth:
        ...
        <div>
          <BootstrapTable
            data={this.state.monthlySalesInfoList}
          ...
          >
            ...
            <TableHeaderColumn
              ...
              dataField="unitPrice"
              width="150"
              dataFormat={this.unitPriceAddComma}
            >
              単価(控除残業)
            </TableHeaderColumn>
            <TableHeaderColumn
            ...
            <TableHeaderColumn
              ...
              dataField="otherFee"
              dataFormat={this.otherFeeAddComma}
              width="100"
            >
              他の費用
            </TableHeaderColumn>
            <TableHeaderColumn
              tdStyle={{ padding: ".45em" }}
              hidden={this.state.bpFlag}
              dataField="waitingCost"
              dataFormat={this.waitingCostAddComma}
              width="110"
            >
              非稼動費用
            </TableHeaderColumn>
