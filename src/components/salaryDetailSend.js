...
class salaryDetailSend extends Component {
  ...
  getSalaryDetail = (type) => {
    if (type === "0") {
      axios
        .post(this.state.serverIP + "SalaryDetailSend/getEmployee")
        .then((response) => {
          this.setState({
      ...
      axios
        .post(this.state.serverIP + "SalaryDetailSend/getEmployeeSameFile")
        .then((response) => {
          this.setState({
            employeeList: response.data.data,
          });
  ...
  selectAll = () => {
    ...
    for (let i in this.state.employeeList) {
      ...
      if (
        !(
          this.state.employeeList[i].fileName === undefined ||
          this.state.employeeList[i].fileName === null ||
          this.state.employeeList[i].fileName === ""
        )
      )
  ...
  handleRowSelect = (row, isSelected, e) => {
    if (isSelected) {
      this.setState({
        ...
        rowEmployeeFristName: this.state.rowEmployeeFristName.concat([
          row.employeeFristName,
        ]),
        ...
        rowFileName:
          ...
          row.fileName === null ||
          row.fileName === ""
            ? this.state.rowFileName
            : this.state.rowFileName.concat([row.fileName]),
      });
    } else {
      let index;
      index = this.state.rowNo.findIndex((item) => item === row.rowNo);
      if (index !== -1) this.state.rowNo.splice(index, 1);

      index = this.state.rowEmployeeNo.findIndex(
        (item) => item === row.employeeNo
      );
      if (index !== -1) this.state.rowEmployeeNo.splice(index, 1);

      index = this.state.rowEmployeeFristName.findIndex(
        (item) => item === row.employeeFristName
      );
      if (index !== -1) this.state.rowEmployeeFristName.splice(index, 1);

      index = this.state.rowCompanyMail.findIndex(
        (item) => item === row.companyMail
      );
      if (index !== -1) this.state.rowCompanyMail.splice(index, 1);

      index = this.state.rowFileName.findIndex((item) => item === row.fileName);
      if (index !== -1) this.state.rowFileName.splice(index, 1);
  ...
  deleteRow = () => {
    ...
    for (let i in this.state.rowNo) {
      let index = employeeList.findIndex(
        (item) => item.rowNo === this.state.rowNo[i]
      );
  ...
  setFileCount = () => {
    ...
    for (let i in employeeList) {
      if (
        !(
          employeeList[i].fileName === undefined ||
          employeeList[i].fileName === null ||
          employeeList[i].fileName === ""
        )
      )
    ...
    if (this.state.format === "0") {
      ...
      for (let i = 0; i < $("#pdfUpdate").get(0).files.length; i++) {
        ...
        for (let j in employeeList) {
          if (
            $("#pdfUpdate").get(0).files[i].name.search(letterStatus) !== -1 &&
            $("#pdfUpdate")
              .get(0)
              .files[i].name.search(String(employeeList[j].employeeNo)) !== -1
          ) {
        ...
        axios
          .post(this.state.serverIP + "SalaryDetailSend/updatePDF", formData)
          .then((result) => {
            if (i === $("#pdfUpdate").get(0).files.length - 1) {
              ...
              if (this.state.fileCount > 0) {
                this.setState({
                  myToastShow: true,
                  myToastShowValue: "取り込み完了",
                });
                setTimeout(() => this.setState({ myToastShow: false }), 3000);
              } else {
                this.setState({
                  errorsMessageShow: true,
                  errorsMessageValue: "取り込み失敗",
                });
                setTimeout(
          ...
          .catch((error) => {
            ...
            this.setState({
              errorsMessageShow: true,
              errorsMessageValue: "アップデートするファイル大きすぎる。",
            });
  ...
  sendState(cell) {
    if (cell === "0") return "未送信";
    else if (cell === "1")
      return <FontAwesomeIcon icon={faCheck} style={{ color: "green" }} />;
    else if (cell === "○") return <div class="donut"></div>;
    else if (cell === "×")
  ...
  sendMailWithFile = () => {
    ...
    let letterYearAndMonth =
      this.state.letterYearAndMonth === "0"
    ...
    for (let i in employeeList) {
      if (
        employeeList[i].companyMail !== undefined &&
        employeeList[i].companyMail !== null &&
        employeeList[i].companyMail !== "" &&
        employeeList[i].sendState !== "1"
      ) {
        var mailConfirmContont =
          employeeList[i].employeeFristName +
          `さん
...
労働者派遣事業許可番号　派13-306371`;
    ...
    axios
      .post(
        this.state.serverIP + "SalaryDetailSend/sendMailWithFile",
        emailModel
      )
      .then((result) => {
        ...
        <Row>
          <Col sm={2}>
            <InputGroup size="sm" className="mb-3">
              ...
              <Form.Control
                ...
                as="select"
                size="sm"
                onChange={this.letterStatusChange}
                name="format"
          ...
          >
            <InputGroup size="sm" className="mb-3">
              ...
              <Form.Control
                id="letterYearAndMonth"
                as="select"
                size="sm"
                onChange={this.valueChange}
                disabled={this.state.format === "1"}
                name="letterYearAndMonth"
                value={this.state.letterYearAndMonth}
                autoComplete="off"
              >
          ...
          <Col sm={5} hidden={!(this.state.format === "1")}></Col>
          ...
          <Col>
            <InputGroup size="sm" className="mb-3">
              ...
              <Autocomplete
                ...
                value={
                  this.state.employeeNameDrop.find(
                    (v) => v.code === this.state.employeeNo
                  ) || {}
                }
                ...
                renderInput={(params) => (
                  <div ref={params.InputProps.ref}>
                    <input
                      ...
                      type="text"
                      {...params.inputProps}
                      className="auto form-control Autocompletestyle-salaryDetailSend"
                    />
                  </div>
                )}
              />
              ...
              <Button
                ...
                variant="info"
                name="clickButton"
                disabled={this.state.employeeNo === null}
          ...
          <Col>
            <div style={{ float: "left" }}>
              <InputGroup size="sm">
                <Button
                  ...
                  disabled={this.state.rowNo.length !== 1}
                  variant="info"
                  id="employeeInfo"
                >
                  個人情報
                ...
                <Button
                  ...
                  disabled={
                    this.state.rowNo.length !== 1 ||
                    this.state.rowFileName.length !== 1
                  }
                >
                  メール確認
                </Button>
            ...
            <div style={{ float: "right" }}>
              <Button
                ...
                variant="info"
                name="clickButton"
                onClick={(event) => this.addFile(event, "pdfUpdate")}
              >
                <FontAwesomeIcon icon={faUpload} /> 取込
              </Button>{" "}
              <Button
                size="sm"
                variant="info"
              ...
              <Button
                ...
                variant="info"
                name="clickButton"
                onClick={this.beforeSendMail}
                disabled={this.state.sendOver}
              >
            ...
            <div hidden>
              <Form.File
                id="pdfUpdate"
                data-browse="添付"
                multiple={this.state.format === "1" ? "" : "multiple"}
                value={this.state.pdfUpdate}
                custom
                onChange={(event) => this.changeFile(event, "pdfUpdate")}
              />
            </div>
          </Col>
        </Row>
          ...
          <div>
            ...
            >
              ...
              <TableHeaderColumn
                ...
                width="10%"
                dataField="sendState"
                dataFormat={this.sendState.bind(this)}
              >
                送信ステータス
              </TableHeaderColumn>
            </BootstrapTable>
